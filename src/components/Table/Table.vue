<template>
    <div ref="tableContainer"
        class="flex flex-col min-h-0 overflow-auto h-full border border-gray-200 dark:border-dark-200 sm:rounded-lg">
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50 sticky z-20 dark:bg-dark-700" style="z-index: 20">
                <tr>
                    <th scope="col"
                        ...
                        }" v-for="header in headers" @click="sortData(header)"
                        ...
                        <div class="flex flex-row items-center">
                            ...
                            <div class="flex flex-col ml-2" v-if="header.enableSorting">
                                <em class="fas fa-caret-up text-gray-400 dark:text-dark-50"
                                    :class="{ 'text-primary': sort && sort.prop === header.key && sort.order === SortType.ASCENDING }"></em>
                                <em class="fas fa-caret-down text-gray-400 dark:text-dark-50"
                                    :class="{ 'text-primary': sort && sort.prop === header.key && sort.order === SortType.DESCENDING }"></em>
                            </div>
                        </div>
                    </th>
                </tr>
            ...
            <tbody class="relative z-10">
                <tr v-if="!isLoading && dataList.length > 0" v-for="(data, index) in dataList" :key="data"
                    class="border-b border-gray-100 dark:border-dark-200" :class="[
                    ...
                    ]" @click.ctrl.exact="e => addItemToSelect(data)" @click.exact="e => selectItem(data)"
                    @click.shift.exact="e => selectRange(data)" @dblclick.stop="e => openItem(data)"
                    :draggable="dragAndDrop ? 'true' : 'false'" @drop.prevent="e => dragDrop(data)"
                    @dragend.prevent="$emit(Emits.StopDragging)" @dragstart="e => dragStart(e, data)"
                    @dragover.prevent="e => dragOver(data)" @drag="e => drag(e)" @dragend="dragEnd">
                    <td class="relative px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white"
                        v-for="header in headers" :data-name="`data-${header.key}`" :key="data[header.key]"
        ...
        <div v-if="isLoading" class="w-full flex flex-row justify-center items-center mt-2">
            <span class="flex flex-col items-center mt-2">
                <svg class="w-8 h-8 animate-spin -ml-1 mr-3 h-5 w-5 text-primary" xmlns="http://www.w3.org/2000/svg"
                    fill="none" viewBox="0 0 24 24">
                    <circle class="text-primary opacity-60" cx="12" cy="12" r="10" stroke="currentColor"
