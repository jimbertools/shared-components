<template>
    <div class="relative inline-block text-left">
        ...
        <ul v-show="isOpen" class="absolute mt-2 left-0 right-0 mb-4 bg-white rounded-sm shadow-lg overflow-hidden">
            <li
                v-for="(option, index) in options"
                :key="`${index}-${option.value}`"
                class="px-3 py-2 transition-colors duration-300 hover:bg-blue-50 hover:text-blue-400 cursor-pointer text-sm text-gray-600"
...
<script lang="ts">
    ...
    export default defineComponent({
        ...
        setup(props, { emit }) {
            ...
            const currentOption = ref<IOption | undefined>(props?.options?.find(x => x.value === props.defaultOption));
